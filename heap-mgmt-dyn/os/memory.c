...
/*
 * Check if a given address falls within an allocated buddy block
 * This is used by the page fault handler to determine if a heap access is legal
 * Returns 1 if the address is within an allocated block, 0 otherwise
 */
 static int is_address_in_allocated_block(BuddyNode *node, uint32 addr_offset)
 {
   if(node == NULL)
   {
     return 0;
   ...
   // If this node is allocated and the address falls within its range
   if(node->is_allocated)
   {
     uint32 blockSize = 1 << node->order;
     if(addr_offset >= node->addr && addr_offset < node->addr + blockSize)
     {
       return 1;
     }
     return 0;
   ...
   // If this node has children, check recursively
   if(node->left_child != NULL || node->right_child != NULL)
   {
     if(node->left_child != NULL)
     {
       if(is_address_in_allocated_block(node->left_child, addr_offset))
       {
         return 1;
       }
     }
     ...
     {
       if(is_address_in_allocated_block(node->right_child, addr_offset))
       {
         return 1;
       }
     }
...
int MemoryPageFaultHandler(PCB *pcb) 
{
  ...
  uint32 heapStartPage = MEM_HEAP_START >> MEM_L1FIELD_FIRST_BITNUM; // Heap starts at 4 pages (4KB)
  uint32 heapEndPage = (MEM_HEAP_START + MEM_HEAP_SIZE-1) >> MEM_L1FIELD_FIRST_BITNUM; // Heap ends around page 19
  ...
  // Check for heap page fault
  dbprintf('m', "MemoryPageFaultHandler: Heap start page: %d, Heap end page: %d\n", heapStartPage, heapEndPage);
  if(vPageNum >= heapStartPage && vPageNum <= heapEndPage)
  {
    // Chec ifi faulting address has already been allocated
    uint32 addrOffset = faultAddr - MEM_HEAP_START;
    dbprintf('m', "MemoryPageFaultHandler: Heap fault detected, checking if address 0x%x (offset %d) is allocated\n",
             faultAddr, addrOffset);

    if(pcb->buddy_root != NULL && is_address_in_allocated_block(pcb->buddy_root, addrOffset))
    {
      ...
      {
        dbprintf('m', "MemoryPageFaultHandler: Failed to allocate page for heap fault, killing process %d\n", 
                 GetPidFromAddress(pcb));
        ProcessKill();
        return MEM_FAIL;
      }
...

      {
        ...
        dbprintf('m', "MemoryPageFaultHandler: Allocated new heap page, vPageNum=%d, physPage=%d, npages=%d\n", 
    ...
    {
      // Address is in the heap range but was never allocated via malloc - SEGFAULT
      dbprintf('m', "MemoryPageFaultHandler: SEGFAULT - Access to unallocated heap memory (vPageNum: %d, offset: %d), killing process %d\n", 
        vPageNum, addrOffset, GetPidFromAddress(pcb));
      printf("SEGFAULT: Process %d accessed unallocated heap memory at address 0x%x\n",
      GetPidFromAddress(pcb), faultAddr);
      ProcessKill();
      return MEM_FAIL;
    }
  }
