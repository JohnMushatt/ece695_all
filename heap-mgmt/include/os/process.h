...
typedef struct PCB {
  ...
  uint32	pagetable[MEM_MAX_VIRTUAL_PAGES]; // Statically allocated page table
  int		npages;		// Number of pages allocated to this process
  uint32 pt_phys_base; // Physical base address of page table, page-aligned
  Link		*l;		// Used for keeping PCB in queues

  // Buddy memory allocator state
  BuddyNode *buddy_root;           // Root of buddy tree for this process
  BuddyNode buddy_node_pool[MAX_BUDDY_NODES];      // Pool of buddy nodes for this process  
  int buddy_node_pool_index;       // Current index in buddy node pool
  //BuddyNode *buddy_free_list;      // Free list of buddy nodes for this process, remove if not needed
  int buddy_initialized;           // Flag indicating if buddy allocator is initialized
