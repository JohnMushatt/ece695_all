...
void ProcessFreeResources (PCB *pcb) {
  ...
  {
    MemoryFreePage(pcb->pt_phys_base >> MEM_L1FIELD_FIRST_BITNUM);
  ...
  for( i = 0; i < pcb->npages; i++)
  {
    ...
    if(pcb->pagetable[i] & MEM_PTE_VALID)
    {
      ...
      dbprintf('m', "Freed page %d, PTE=0x%x, physical page=%d.\n", i, pte, physicalPage);
    }
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  }
  {
    ...
    if( physPage == MEM_FAIL)
    {
      dbprintf('m', "ProcessFork: Failed to allocate page for buddy allocator, killing process %d\n", GetPidFromAddress(pcb));
      ProcessFreeResources(pcb);
      return (-1);
    }
    pcb->pagetable[4] = MemorySetupPte(physPage);
    pcb->npages += 1;
    dbprintf('m', "ProcessFork: Allocated page %d for buddy heap.\n", physPage);
