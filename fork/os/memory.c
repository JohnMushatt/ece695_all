...
static int page_ref_count[MEM_MAX_PHYS_PAGES];
...
void MemoryModuleInit() {
  ...
  for(i = 0; i < MEM_MAX_PHYS_PAGES;i++)
  {
    page_ref_count[i] = 0;
  }
  dbprintf('m', "Initialized page reference count array...\n");
...
int MemoryCopyOnWriteHandler(PCB *pcb)
{
  uint32 *tempStackFrame = pcb->currentSavedFrame;
  uint32 faultAddr = tempStackFrame[PROCESS_STACK_FAULT];
  uint32 vPageNum = faultAddr >> MEM_L1FIELD_FIRST_BITNUM;
  uint32 pte = pcb->pagetable[vPageNum];
  uint32 physAddr = pte & MEM_PTE_MASK;
  uint32 oldPhysPageNum = physAddr >> MEM_L1FIELD_FIRST_BITNUM;
  int newPhysPageNum = 0;
  uint32 *physPageTable = (uint32 *)pcb->pt_phys_base;

  if( !( pte & MEM_PTE_READONLY))
  {
    dbprintf('m', "MemoryCopyOnWriteHandler: Page %d is not read only, killing process %d\n", 
             vPageNum, GetPidFromAddress(pcb));
    ProcessKill();
    return MEM_FAIL;
  }
  // If the page is only referenced by this process, we can just mark it as writable
  if( MemoryGetPageRefCount(oldPhysPageNum) == 1)
  {
    pcb->pagetable[vPageNum] &= ~MEM_PTE_READONLY; // Remove the read only bit
    physPageTable[vPageNum] = pcb->pagetable[vPageNum]; // Copy the updated PTE to physical page table
    dbprintf('m', "MemoryCopyOnWriteHandler: Page %d is now writable, npages=%d (single owner)\n", 
             vPageNum, pcb->npages);
    return MEM_SUCCESS;
  }

  newPhysPageNum = MemoryAllocPage();
  if(newPhysPageNum == MEM_FAIL)
  {
    dbprintf('m', "MemoryCopyOnWriteHandler: Failed to allocate new page for page %d, killing process %d\n", 
             vPageNum, GetPidFromAddress(pcb));
    ProcessKill();
    return MEM_FAIL;
  }

  {
    ...
    uint32 oldPhysAddr  = oldPhysPageNum << MEM_L1FIELD_FIRST_BITNUM;
    uint32 newPhysAddr  = newPhysPageNum << MEM_L1FIELD_FIRST_BITNUM;
    bcopy((char *)oldPhysAddr, (char *)newPhysAddr, MEM_PAGESIZE);

    MemoryDecrementPageRefCount(oldPhysPageNum);
    // Update the PTE for the new page
    newPTE = (newPhysAddr | MEM_PTE_VALID);
    newPTE &= ~MEM_PTE_READONLY;
    
    
    
    pcb->pagetable[vPageNum] = newPTE;
    physPageTable[vPageNum] = newPTE;
    dbprintf('m', "MemoryCopyOnWriteHandler: Page %d is now writable, npages=%d (multiple owners, decremented ref count of old page %d)\n", 
             vPageNum, pcb->npages, oldPhysPageNum);
...
int MemoryAllocPage(void) {
  ...
  if(nfreepages == 0)
  {
    dbprintf('m', "MemoryAllocPage: No free pages available (nfreepages=%d).\n", nfreepages);
    return MEM_FAIL;
  }
  ...
  MemoryIncrementPageRefCount(pagenum);
  nfreepages -= 1;
...
void MemoryFreePage(uint32 page) {
  ...
  if( page>= pagestart && page < MEM_MAX_PHYS_PAGES)
  {
    ...
    page_ref_count[page] = 0;  // Reset ref count when freed
...
void MemoryIncrementPageRefCount(uint32 page) {
  page_ref_count[page]++;
}

void MemoryDecrementPageRefCount(uint32 page) {
  page_ref_count[page]--;
}

int MemoryGetPageRefCount(uint32 page) {
