...
void ProcessModuleInit () {
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    ...
    pcbs[i].pt_phys_base = 0;
    pcbs[i].npages = 0;
    for(j = 0; j < MEM_MAX_VIRTUAL_PAGES; j++)
    {
      pcbs[i].pagetable[j] = 0;  // All pages initially invalid
    }
    dbprintf('m', "Initialized PCB %d, page table=0x%x, npages=%d.\n", i, (int)pcbs[i].pagetable, pcbs[i].npages);
    ...
    dbprintf('m', "Initialized PCB %d, pt_phys_base=0x%x, npages=%d.\n", i, pcbs[i].pt_phys_base, pcbs[i].npages);
  }
...
void ProcessFreeResources (PCB *pcb) {
  ...
  //------------------------------------------------------------
  // Loop through process's pages and free them
  if(pcb->pt_phys_base != 0)
  {
    uint32 ptPageNum = pcb->pt_phys_base >> MEM_L1FIELD_FIRST_BITNUM;
    MemoryDecrementPageRefCount(ptPageNum);  // ADD THIS LINE
    MemoryFreePage(ptPageNum);
    dbprintf('m', "Freed page table for process %d.\n", GetPidFromAddress(pcb));
    pcb->pt_phys_base = 0;
  }
  for( i = 0; i < pcb->npages; i++)
  {
    // If page has valid PTE, free it
    if(pcb->pagetable[i] & MEM_PTE_VALID)
    {
      int refCount = 0;
      // Grab PTE for the page
      pte = pcb->pagetable[i];
      // Get the physical address of the page
      phyAddr = pte & MEM_PTE_MASK;
      // Get the physical page number
      physicalPage = phyAddr >> MEM_L1FIELD_FIRST_BITNUM;
      
      MemoryDecrementPageRefCount(physicalPage);
      refCount = MemoryGetPageRefCount(physicalPage);
      if(refCount == 0)
      {
        MemoryFreePage(physicalPage);
        dbprintf('m', "Freed page %d, PTE=0x%x, physical page=%d (ref count = %d).\n", i, pte, physicalPage, refCount);
      }
      else
      {
        dbprintf('m', "Decremented ref count of page %d, PTE=0x%x, physical page=%d (ref count = %d).\n", i, pte, physicalPage, refCount);
      }
    }

  }
  // Clear the page table
  for(i = 0; i < MEM_MAX_VIRTUAL_PAGES; i++)
  {
    pcb->pagetable[i] = 0;
  }
...
int ProcessRealFork(void)
{
  ...
  int i =0;
  uint32 *stackframe = 0;
  uint32 *childPhysPageTable = 0;
  uint32 *parentPhysPageTable = 0;
  uint32 pte = 0;
  PCB *parentPCB = currentPCB;
  ...
  // Check for a free PCB to see if we can spawn a child process
  if(AQueueEmpty(&freepcbs))
  {
    printf("FATAL error: no free processes!\n");
    exitsim();
  }
  dbprintf('m', "ProcessRealFork: got a free PCB @ 0x%x\n", (int)(pcb->l));
  ...
  dbprintf('m', "ProcessRealFork: set child PCB to @ 0x%x\n", (int)(childPCB));

  ProcessSetStatus(childPCB, PROCESS_STATUS_RUNNABLE);
  dbprintf('m', "ProcessRealFork: set child PCB status to RUNNABLE\n");

  RestoreIntrs(intrs);



  // Copy parent PCB over to child PCB
  bcopy( (char *) parentPCB, (char *) childPCB, sizeof(PCB));
  dstrcpy(childPCB->name, parentPCB->name);


  // Allocatge physical page for Page Table of child PCB

  ptPage = MemoryAllocPage();
  if(ptPage == MEM_FAIL)
  {
    printf("FATAL error: could not allocate page for page table in ProcessRealFork!\n");
    exitsim();
  }
  childPCB->pt_phys_base = ptPage << MEM_L1FIELD_FIRST_BITNUM;
  bzero((char *)childPCB->pt_phys_base, MEM_PAGESIZE);

  childPhysPageTable = (uint32 *)childPCB->pt_phys_base;
  parentPhysPageTable = (uint32 *)parentPCB->pt_phys_base;
  dbprintf('m', "ProcessRealFork: child vs parent page tables: %p vs %p\n", (void *)childPhysPageTable, (void *)parentPhysPageTable);
  for(i = 0; i < MEM_MAX_VIRTUAL_PAGES; i++)
  {

    pte = parentPCB->pagetable[i];
    // If PTE is valid, copy it to child page table
    if( pte & MEM_PTE_VALID)
    {
      // Need to updatre reference count of page
      uint32 page = pte & MEM_PTE_MASK;
      uint32 physPageNum = page >> MEM_L1FIELD_FIRST_BITNUM;


      // Since both parent child reference same page, mark as Read Only 
      uint32 newPTE = pte | MEM_PTE_READONLY;
      
      parentPCB->pagetable[i] = newPTE;
      parentPhysPageTable[i] = newPTE;

      childPCB->pagetable[i] = newPTE;
      childPhysPageTable[i] = newPTE;

      MemoryIncrementPageRefCount(physPageNum);
      dbprintf('m', "ProcessRealFork: Shared page %d (vpage=%d) between parent %d and child\n",
        physPageNum, i, GetCurrentPid());    }
  }


  {
    int sysStackPageNum = MemoryAllocPage();
    if(sysStackPageNum == MEM_FAIL)
    {
      printf("FATAL error: could not allocate page for system stack in ProcessRealFork!\n");
      ProcessFreeResources(childPCB);
      exitsim();
    }
    childPCB->sysStackArea = sysStackPageNum * MEM_PAGESIZE;

    stackframe = ((uint32 *)(childPCB->sysStackArea + MEM_PAGESIZE)) - (PROCESS_STACK_FRAME_SIZE + 8);
    
    bcopy((char *)parentPCB->currentSavedFrame, (char *)stackframe, PROCESS_STACK_FRAME_SIZE * sizeof(uint32));
    childPCB->sysStackPtr = stackframe;
    childPCB->currentSavedFrame = stackframe;

    stackframe[PROCESS_STACK_PTBASE] = (uint32)childPCB->pt_phys_base;
    stackframe[PROCESS_STACK_PTSIZE] = (uint32)MEM_MAX_VIRTUAL_PAGES;  // 1024 pages
    stackframe[PROCESS_STACK_PTBITS] = (uint32)((MEM_L1FIELD_FIRST_BITNUM << 16) | MEM_L1FIELD_FIRST_BITNUM);
    dbprintf('m', "ProcessRealFork: PTBASE=0x%08x PTSIZE=%lu PTBITS=%d\n",
      (uint32)stackframe[PROCESS_STACK_PTBASE],
      (uint32)stackframe[PROCESS_STACK_PTSIZE],
      (uint32)stackframe[PROCESS_STACK_PTBITS]);
    dbprintf('m', "ProcessRealFork: Copied system stack from parent to child\n");

    //stackframe[PROCESS_STACK_USER_STACKPOINTER] = (MEM_MAX_VIRTUAL_PAGES - 1) << MEM_L1FIELD_FIRST_BITNUM;
    //stackframe[PROCESS_STACK_USER_STACKPOINTER] |= (MEM_PAGESIZE - 4);  // Highest 4-byte aligned address
    ProcessSetResult(childPCB, 0);

    intrs = DisableIntrs();
    if( (childPCB->l = AQueueAllocLink(childPCB)) == NULL)
    {
      printf("FATAL error: could not get link for child PCB in ProcessRealFork!\n");
      ProcessFreeResources(childPCB);
      exitsim();
    }
    ...
    RestoreIntrs(intrs);
    cid = childPCB - pcbs;
    //childPCB->pagetable[MEM_MAX_VIRTUAL_PAGES - 1] = MemorySetupPte(sysStackPageNum);
  }
  // Handle memory and page tables
  dbprintf('m', "ProcessRealFork: Parent (pid=%d) created child (pid=%d) successfully\n", GetCurrentPid(), cid);
  return cid;
}
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
 ...
 
  {
    ...
    // Map 4 pages for code and data, vPageNum 0...3
    for (i = 0; i < 4; i++) 
    {
      physPage = MemoryAllocPage();
      if( physPage == MEM_FAIL)
      {
        dbprintf('m', "ProcessFork: Failed to allocate page for code and data, killing process %d\n", GetPidFromAddress(pcb));
        ProcessFreeResources(pcb);
        return (-1);
      }
      // Setup PTE for code and data
      pcb->pagetable[i] = MemorySetupPte(physPage);
      pcb->npages += 1;
      dbprintf('m', "ProcessFork: Allocated page %d for code and data.\n", physPage);
    }
    dbprintf('m', "ProcessFork: Allocated %d pages for code and data.\n", MEM_INIT_CODE_PAGES);
  }
  {
    // Top of virtual address space for user stack
    page = MEM_MAX_VIRTUAL_PAGES - 1;
    physPage = MemoryAllocPage();
    if( physPage == MEM_FAIL)
    {
      dbprintf('m', "ProcessFork: Failed to allocate page for user stack, killing process %d\n", GetPidFromAddress(pcb));
      ProcessFreeResources(pcb);
      return (-1);
    }
    // Setup PTE for user stack
    pcb->pagetable[page] = MemorySetupPte(physPage);
    dbprintf('m', "ProcessFork: Allocated page %d for user stack.\n", physPage);
  }
  {
    int newPage = 0;
    ...
    if( newPage == MEM_FAIL)
    {
      dbprintf('m', "ProcessFork: Failed to allocate page for system stack %d\n", GetPidFromAddress(pcb));
      exitsim();
    }
    //pcb->pagetable[0] = MemorySetupPte(newPage);
    pcb->sysStackArea = newPage * MEM_PAGESIZE;
    //pcb->npages += 1;
    dbprintf('m', "ProcessFork: Allocated page %d for process %d, sysStackArea=0x%08x.\n", newPage, GetPidFromAddress(pcb), pcb->sysStackArea);
  }


  {
    uint32 *physPageTable = 0;
    int i = 0;
      // Allocate a physical page for the page table
    int ptPage = MemoryAllocPage();
    if(ptPage == MEM_FAIL)
    {
      dbprintf('m', "ProcessFork: Failed to allocate page for page table\n");
      ProcessFreeResources(pcb);
      return (-1);
    }
   
    // Set the physical base address of the page table
    pcb->pt_phys_base = ptPage << MEM_L1FIELD_FIRST_BITNUM;
    bzero((char *)pcb->pt_phys_base, MEM_PAGESIZE);

    // Copy the page table entries to the physical page table
    physPageTable = (uint32 *)pcb->pt_phys_base;
    for(i = 0; i < 4; i++)
    {
      dbprintf('m', "ProcessFork: BEFORE: physPageTable[%d] = 0x%08x (from pagetable[%d] = 0x%08x)\n", 
             i, physPageTable[i], i, pcb->pagetable[i]);
      physPageTable[i] = pcb->pagetable[i];
      dbprintf('m', "ProcessFork: AFTER: physPageTable[%d] = 0x%08x (from pagetable[%d] = 0x%08x)\n", 
             i, physPageTable[i], i, pcb->pagetable[i]);
    }
    physPageTable[MEM_MAX_VIRTUAL_PAGES - 1] = pcb->pagetable[MEM_MAX_VIRTUAL_PAGES - 1];

    dbprintf('m', "ProcessFork: Allocated page table at physical address 0x%08x\n", pcb->pt_phys_base);
  }
  {
    stackframe = ((uint32 *)(pcb->sysStackArea + MEM_PAGESIZE)) - (PROCESS_STACK_FRAME_SIZE + 8);
    dbprintf('m', "ProcessFork: Stackframe set to 0x%08x.\n", (int)stackframe);
  ...
  // stack frame.
  //----------------------------------------------------------------------
  stackframe[PROCESS_STACK_PTBASE] = (uint32)pcb->pt_phys_base;
  stackframe[PROCESS_STACK_PTSIZE] = (uint32)MEM_MAX_VIRTUAL_PAGES;  // 1024 pages
  stackframe[PROCESS_STACK_PTBITS] = (uint32)((MEM_L1FIELD_FIRST_BITNUM << 16) | MEM_L1FIELD_FIRST_BITNUM);  
  /*
  dbprintf('m', "\nPTBASE=0x%08x PTSIZE=%lu PTBITS=%lu\n",
    (unsigned long)stackframe[PROCESS_STACK_PTBASE],
    (unsigned long)stackframe[PROCESS_STACK_PTSIZE],
    (unsigned long)stackframe[PROCESS_STACK_PTBITS]);
  */

  //stackframe[PROCESS_STACK_PTSIZE] = MEM_MAX_VIRTUAL_PAGES;
  if (isUser) {
    ...
    while ((n = ProcessGetFromFile (fd, buf, &addr, sizeof (buf))) > 0) {
      dbprintf ('p', "Placing %d bytes at vaddr 0x%08x.\n", n, addr - n);
    ...
    stackframe[PROCESS_STACK_USER_STACKPOINTER] = (MEM_MAX_VIRTUAL_PAGES - 1) << MEM_L1FIELD_FIRST_BITNUM;
    stackframe[PROCESS_STACK_USER_STACKPOINTER] |= (MEM_PAGESIZE - 4);  // Highest 4-byte aligned address
    //stackframe[PROCESS_STACK_USER_STACKPOINTER] = (MAX_VIRTUAL_ADDRESS & ~0x3); // 4-byte aligned
    //--------------------------------------------------------------------
  ...
  dbprintf('m', "\nPTBASE=0x%08x PTSIZE=%lu PTBITS=%lu\n",
    (unsigned long)stackframe[PROCESS_STACK_PTBASE],
    (unsigned long)stackframe[PROCESS_STACK_PTSIZE],
    (unsigned long)stackframe[PROCESS_STACK_PTBITS]>> 16);
