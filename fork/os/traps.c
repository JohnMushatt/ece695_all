	     ...
	     uint32 *trapArgs)
{
  ...
  if (cause & TRAP_TRAP_INSTR) {
    ...
    case TRAP_PROCESS_FORK:
      ...
      {
        int cID = ProcessRealFork();
        dbprintf('m', "TRAP_FORK: cID = %d\n", cID);
        ProcessSetResult(currentPCB, cID);
    ...
    case TRAP_ADDRESS:
      ...
      break;
    case TRAP_READONLY:
      dbprintf ('t', "Got a readonly page fault trap!\n");
      MemoryCopyOnWriteHandler(currentPCB);
      break;
    ...
    case TRAP_PAGEFAULT:
      ...
      break;
    default:
      printf ("Got an unrecognized system interrupt (0x%x) - exiting! No trap bit set.\n",
	      cause);
      exitsim ();
      break;
    }
